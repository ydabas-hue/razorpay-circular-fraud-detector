...
2. **Agentic SAR Classification (`sar_agent.py`):** Interfaces with NVIDIA NIM APIs (Llama 3.2 11B) using constrained JSON prompts to evaluate metadata signals, IP subnet overlap, and merchant category code (MCC) alignment.
...
[ Transaction Ledger & KYC Data ]
│
▼
[ Graph Engine (NetworkX) ] ──(Discovers 3-Hop Loops)
│
▼
[ NVIDIA NIM LLM Agent ]  ──(Analyzes Metadata & Subnets)
│
┌──────┴──────┐
▼             ▼
[ FRAUD ]      [ LEGITIMATE ]
│
▼
[ Mock API Payout Freeze & RBI SAR Filing ]
